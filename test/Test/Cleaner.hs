...
cleanAppState :: TestEnv -> (PG.Connection -> IO a) -> IO a
cleanAppState TestEnv{..} f = do
  cleanLogs testMessages
  IO.liftIO $ Reader.runReaderT (cleanDBBefore f) testEnvironment
